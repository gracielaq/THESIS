...
public class Twokenizer {
	...
	private static String noseArea = "(|o|O|-)"; // rather tight precision, \\S
													// might be reasonable...
	private static String happyMouths = "[D\\)\\]]";
	private static String sadMouths = "[\\(\\[]";
	private static String tongue = "[pP]";
	private static String otherMouths = "[doO/\\\\]"; // remove forward slash if
														// http://'s aren't
														// cleaned

	private static String emoticon = "(" + normalEyes + "|" + wink + ")" + noseArea + "(" + tongue + "|" + otherMouths
	...
	public Vector<String> splitToken(String token) {
		...
		while (contractionsMatcher.find()) {
			// There should be only two groups in a match for Contractors
			// pattern
	...
	public List<String> simpleTokenize(String text) {
		...
		// XXX: I do this since I couldn't find an equivalent method in Java
		// which does
		...
		// split based on special patterns (like contractions) and remove all
		// tokens are empty
	...
	// the twokenize method which filters out white spaces before using
	// simpleTokenize()
		...
		for (File file : folder.listFiles(new FilenameFilter() {
			...
			try {
				...
				while (s.hasNextLine()) {
					...
					List<String> tokens = twokenizer.twokenize(str);
